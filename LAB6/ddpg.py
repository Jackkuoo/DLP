...
def train(args, env, agent, writer):
	...
	for episode in range(args.episode):
		...
		for t in itertools.count(start=1):
			...
			total_reward += reward
			total_steps += 1
			#print(f'Step: {total_steps}\tEpisode: {episode}\tLength: {t:3d}\tTotal reward: {total_reward:.2f}')
			if done:
				...
				y.append(ewma_reward)
				# z.append(total_reward)
	...
	plt.plot(x, y, marker='.')
	# plt.plot(x, z, marker='.',label=f'episode_reward')
	plt.grid('on')
	# plt.legend()
	plt.title('DDPG LunarLanderContinuous-v2', fontsize=15)
	plt.ylabel('ewma reward', fontsize=15)
...
def main():
    ...
    parser.add_argument('--episode', default=3000, type=int)#1200
