...
if __name__ == '__main__':
	...
	"""
	resnet18/50 without pretrained weights
	"""
	model_wo=ResNet50(num_class=num_class,pretrained=False) #change 18 or 50
	...
	"""
	resnet18/50 with pretrained weights
	    feature extraction for few epochs, then finefuning for some epochs
	"""
	model_with=ResNet50(num_class=num_class,pretrained=True) #change 18 or 50
	# feature extraction
	params_to_update=[]
	...
	model.load_state_dict(torch.load(os.path.join('other','resnet18_with_pretraining.pt')))
	model=model.to(device)
	_,acc=evaluate(model,loader_test,device,num_class=5)
	print("ResNet18 Testing accuracy:",acc)

	dataset_test=RetinopathyDataSet(img_path='data',mode='test')
	loader_test=DataLoader(dataset=dataset_test,batch_size=8,shuffle=False,num_workers=4)
	model=ResNet50(num_class=5,pretrained=True)
	model.load_state_dict(torch.load(os.path.join('other','resnet50_with_pretraining.pt')))
	model=model.to(device)
	_,acc=evaluate(model,loader_test,device,num_class=5)
	print("ResNet50 Testing accuracy:",acc)
